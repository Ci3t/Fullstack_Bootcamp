...
const getAgencyByName = (agenciesArr, name) => {
  ...
  const agentName = []
  agenciesArr.forEach(el => {
    if (el.agencyName == name) {
      agentName.push(el)
    }
  })
...
// console.log(getAgencyById(ourCarMarket.sellers,'Plyq5M5AZ'));
...
console.log(getAllAgenciesNameAndId('Plyq5M5AZ', 'Best Deal'));
//* 4. getCarsToBuy
//? @param {array}    - agenciesArr
...
const getCarsToBuyByModel = (agenciesArr, model) => {
  ...
  // console.log(getCarsToBuyByModel());
  // };
  ...
  const getCarsToBuyByAgencyId = (agenciesArr, agencyId) => {
    // console.log(agenciesArr[0].cars);
    let agent = []
    agenciesArr.forEach(e => {
      const car = e.cars
      for (let i in car) {
    ...
    // console.log(agent);
    return agent.filter(e => e.ownerId === agencyId)
    // return agent

  };
  // console.log(getCarsToBuyByAgencyId(ourCarMarket.sellers,'gNHjNFL12'));
...
const getCarsToBuyByModelAndAgencyId = (agenciesArr, model, agencyId) => {
  ...
  // console.log(agent);
  return agent.filter(e => e.ownerId === agencyId && e.name === model)
  // return agent

};
// console.log(getCarsToBuyByModelAndAgencyId(ourCarMarket.sellers,'Corolla','gNHjNFL12'));
...
const getAllCustomerCars = (customersArr, customerId) => {
  ...
  return customersArr.filter((el) => {
    ...
    return cars.filter((e => {
      return e.ownerId.includes(customerId)
    }))

  });

  // let agent = []
  // customersArr.forEach(e=>{
  //  const car = e.cars
  // //  console.log(car);
  //   for(let i of car){
  ...
  // })
  // // console.log(agent);
  // return agent.filter(e=> e.ownerId === customerId)
  // return agent
};
...
const setNewCarToAgency = (agencyObject, carObject) => {
  ...
  return agencyObject.filter((el) => {
    // console.log(el.cars);
    const cars = el.cars
    console.log(cars);
    cars.Honda = carObject


  })
}

  console.log(setNewCarToAgency(ourCarMarket.sellers, {
    name: 'Civic', year: 2012, price: 548100, carNumber: 'EMW_123', ownerId: "xNHjNF32s"
  }));
  ...
  console.log(deleteCarFromAgency(ourCarMarket.sellers, 'AZJZ4'));
