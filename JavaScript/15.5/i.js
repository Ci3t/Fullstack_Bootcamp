...
const filter1 =(arr , rating)=>{
    
    
    let y = arr.filter(a => {
        return a.rating.average > rating
        
    })
    return y

}
console.log(filter1(movies, 7));
...
const run = (arr , rating , time) =>{

    let x = arr.filter(b => {

        return b.rating.average > rating && b.runtime <= time;
        
    })
    return x
}

console.log(run(movies , 8 , 60));
...
const run2 = (arr , rating1 , time) =>{
    let c = [];
    arr.filter(({name,rating,runtime}) => {
        if(rating.average > rating1 && runtime <= time){
           c = {name,rating};    
        } 
    });
    return c
